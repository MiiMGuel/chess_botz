...
int main(int argc, char* argv[]) {
    ...
    if (close != 0) 
        logg_print(LOGG_WARN, "atexit(_entry_close) failed!");

    if (_entry_app.start != NULL) _entry_app.start(_entry_app.app_data);
    if (_entry_app.run != NULL) _entry_app.run(_entry_app.app_data);
    return 0;
}